...
import AnimatedMetricCard from "@/components/data-explorer/AnimatedMetricCard"
import { Activity, Zap, Gauge } from "lucide-react"

const DataExplorerV3a: React.FC = () => {
  // Sample run date
        ...
        <div className="mt-6">
          ...
          <div className="grid grid-cols-3 gap-3 mb-6">
            <AnimatedMetricCard
              ...
              unit={selectedMetric.includes("force") ? "N" : selectedMetric.includes("load") ? "%" : "°"}
              change={5.2}
              icon={<Activity size={20} className="text-[#42b4f7]" />}
            />
            <AnimatedMetricCard
              title="Peak"
              value={getCurrentValue() * 1.3}
              unit={selectedMetric.includes("force") ? "N" : selectedMetric.includes("load") ? "%" : "°"}
              change={-2.1}
              icon={<Zap size={20} className="text-[#42b4f7]" />}
            />
            <AnimatedMetricCard
              title="Stability"
              value={85}
              unit="%"
              change={3.7}
              icon={<Gauge size={20} className="text-[#42b4f7]" />}
            />
          </div>
          ...
          <div className="bg-[#f0f7ff] p-3 rounded-md mb-6">
            <p className="text-sm text-[#42b4f7] font-medium">Interaction Tip</p>
            <p className="text-xs text-gray-600 mt-1">
              Tap the sparkle icon ✨ next to the chart title to get an AI-powered explanation of your biomechanical
              data. You can also tap directly on data points for specific insights.
            </p>
          </div>
        </div>
      </div>
...
export default DataExplorerV3a
